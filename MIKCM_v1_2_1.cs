...
namespace MadIslandKCM
{
    ...
    public static class DirectGameDataModifier
    {
        ...
        "drawui", "updatebar", "refresh", "statuschange"
        };
        ...
        public static void RefillHungerAndThirst()
        {
            ...
            // FIX BUGS 1 & 2: Используем РАЗНЫЕ visited хэшсеты для еды и жажды!
            // Раньше один и тот же visited блокировал проверку жажды.
            ...
            foreach (var comp in components)
            {
                ...
                // 1) Заполнение ЕДЫ (Голод)
                HashSet<object> visitedHunger = new HashSet<object>();
                InspectAndModifyCustomNames(comp, 9999f, hungerNames, 0, visitedHunger);

                // Прямой доступ к полям макс. значения еды если возможно
                float maxF = GetValueFromFields(comp, new string[] { "maxfood", "max_food", "maxhunger", "max_hunger", "maxsatiety" });
                if (maxF > 0)
                {
                    InspectAndModifyCustomNames(comp, maxF, hungerNames, 0, new HashSet<object>());
                }

                // 2) Заполнение ЖАЖДЫ (Вода) - РЕШЕНИЕ БАГА №1
                HashSet<object> visitedThirst = new HashSet<object>();
                ...
                InspectAndModifyCustomNames(comp, targetWaterVal, thirstNames, 0, visitedThirst);

                // 3) Вызов встроенных методов восполнения еды/жажды
            ...
            // Прямое обращение через структуры Mad Island если они доступны в домене
            try
            {
                GameObject managersObj = GameObject.Find("Managers");
                if (managersObj != null)
                {
                    Component mnComp = managersObj.GetComponent("ManagersScript");
                    if (mnComp != null)
                    {
                        FieldInfo gameMNField = mnComp.GetType().GetField("gameMN");
                        if (gameMNField != null)
                        {
                            object gameMN = gameMNField.GetValue(mnComp);
                            if (gameMN != null)
                            {
                                FieldInfo playerField = gameMN.GetType().GetField("player");
                                GameObject playerGO = (playerField != null) ? playerField.GetValue(gameMN) as GameObject : null;
                                if (playerGO != null)
                                {
                                    Component common = playerGO.GetComponent("CommonStates");
                                    if (common != null)
                                    {
                                        FieldInfo maxFoodF = common.GetType().GetField("maxFood");
                                        FieldInfo foodF = common.GetType().GetField("food");
                                        FieldInfo maxWaterF = common.GetType().GetField("maxWater");
                                        FieldInfo waterF = common.GetType().GetField("water");

                                        if (maxFoodF != null && foodF != null)
                                        {
                                            float mf = Convert.ToSingle(maxFoodF.GetValue(common));
                                            foodF.SetValue(common, mf > 0 ? mf : 100f);
                                        }
                                        if (maxWaterF != null && waterF != null)
                                        {
                                            float mw = Convert.ToSingle(maxWaterF.GetValue(common));
                                            waterF.SetValue(common, mw > 0 ? mw : 100f);
                                        }
                                    }
                                }
                            }
            ...
            // Мгновенное обновление UI (РЕШЕНИЕ БАГА №2)
            ForceUIRefresh();
        }
        ...
        {
            // РЕШЕНИЕ БАГА №2: Прямой вызов игрового интерфейса
            try
            {
                GameObject managersObj = GameObject.Find("Managers");
                if (managersObj != null)
                {
                    ...
                    if (mnComp != null)
                    {
                        FieldInfo uiMNField = mnComp.GetType().GetField("uiMN");
                        FieldInfo gameMNField = mnComp.GetType().GetField("gameMN");

                        if (uiMNField != null && gameMNField != null)
                        {
                            object uiMN = uiMNField.GetValue(mnComp);
                            object gameMN = gameMNField.GetValue(mnComp);

                            if (uiMN != null && gameMN != null)
                            {
                                FieldInfo playerField = gameMN.GetType().GetField("player");
                                GameObject playerGO = (playerField != null) ? playerField.GetValue(gameMN) as GameObject : null;
                                ...
                                if (playerGO != null)
                                {
                                    ...
                                    if (common != null)
                                    {
                                        // Главный метод игры для моментальной перерисовки полосок HUD
                                        MethodInfo statusChangeMethod = uiMN.GetType().GetMethod("StatusChange", BindingFlags.Public | BindingFlags.Instance);
                                        if (statusChangeMethod != null)
                                        {
                                            statusChangeMethod.Invoke(uiMN, new object[] { common });
                                        }
                                    }
                                }
                            }
                        ...
                        foreach (var m in methods)
                        {
                            ...
                            if (refreshMethodNames.Contains(mNameLower))
                            {
                                try
                                {
                                    ...
                                    {
                                        GameObject pObj = GetPlayerObject();
                                        if (pObj != null)
                                        {
                                            Component cs = pObj.GetComponent("CommonStates");
                                            if (cs != null && paramsInfo[0].ParameterType.IsAssignableFrom(cs.GetType()))
                                            {
                                                m.Invoke(comp, new object[] { cs });
                                            }
                                        }
                                    }
                                }
        ...
        private static bool SetPropertyMemberValue(PropertyInfo prop, object target, float val)
        {
            try
            {
                ...
                if (prop.PropertyType == typeof(int)) { prop.SetValue(target, (int)val, null); return true; }
